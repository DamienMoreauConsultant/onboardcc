-- Variante compatible PostgreSQL 9.6 (o2switch), generee automatiquement le 08/09/2026
-- a partir de schema_only_20260908_101006.sql (source : PostgreSQL 16.10, Replit).
-- Modifications : suppression des marqueurs \restrict/\unrestrict (pg_dump 17+, inconnus
-- de psql 9.6), suppression de 'SET default_table_access_method' (PG12+),
-- suppression de la clause 'AS integer' des CREATE SEQUENCE (PG10+, non supportee en 9.6 --
-- la sequence garde son comportement par defaut, sans consequence pratique),
-- suppression des ALTER ... OWNER TO postgres (role inexistant sur ce serveur -- les objets
-- restent la propriete de l'utilisateur applicatif, ce qui est le comportement voulu).
